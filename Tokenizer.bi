''----------------------------------------------------------------------------------------------------------------------
'' Tokenizer.bi version 1.03
'' #include once "Tokenizer.bi"
''----------------------------------------------------------------------------------------------------------------------
Namespace Tokenizer
	Function load_file(ByRef file_name As String) As boolean
		'
		Dim As Integer fnum 
		fnum = FreeFile()
		...
		EndIf
			...
			_text = StrPtr(source_text)
		Close fnum
		Return TRUE  
	 	
	End Function
	Sub _tokenize_EOL(ByRef add_end_of_statement_token As boolean = TRUE)
		'
		Dim As ULongInt tlen = Len(*_text)
		Dim As ZString Ptr z = _text
		Dim As ULongInt Ptr n = @_char_index
		...
		_char_col = 1
	End Sub
	Sub _tokenize_suffix(ByRef t_text As TCharString, ByRef t As TToken, _
												ByRef this_char As Byte, ByRef is_hex As Const boolean = FALSE)
		...
		Else
			...
			Cls
			? "Invalid suffix char '" & this_char & "' in code at line " & _char_line 
			? __ERROR_INFO  
			Sleep
			End
		EndIf
	...
	End Sub 	
	Sub _tokenize_sci_note(ByRef t_text As TCharString, ByRef t As TToken, ByRef this_char As Byte)
		'
		Dim As ULongInt tlen = Len(*_text)
		Dim As ZString Ptr z = _text
		Dim As ULongInt Ptr n = @_char_index
		...
		*n += 1
		While *n < tlen
			...
			ElseIf __TS.is_sci_note(this_char) = TRUE Then 		' e/d - start of sci note exponent
	   		_tokenize_sci_note(t_text, t, this_char)
	   		Exit While 
	
			ElseIf __TS.is_suffix_char(this_char, FALSE) = TRUE Then	
				_tokenize_suffix(t_text, t, this_char, FALSE) 
				Exit While 
	...
	End Sub
	Function _is_binary_string(ByRef t As TToken) As boolean
		'
		Dim As ZString Ptr z = _text
		Dim As Byte this_char
		Dim As ULongInt Ptr n = @_char_index
		...
		*n += 1
		...
		While *n < tlen
			...
			If __TS.is_suffix_char(this_char, FALSE) = TRUE Then 
				...
				_tokenize_suffix(t_text, t, this_char, FALSE) ' the false means its not hex 
				Return TRUE 
	...
	End Function
	Function _is_oct_string(ByRef t As TToken) As boolean
		'
		Dim As ZString Ptr z = _text
		Dim As Byte this_char
		Dim As ULongInt Ptr n = @_char_index
		...
		*n = orig_n
		Return FALSE 
	End Function
	Function _is_hex_string(ByRef t As TToken) As boolean
		'
		Dim As ZString Ptr z = _text
		Dim As Byte this_char
		Dim As ULongInt Ptr n = @_char_index
		...
		*n += 1
		...
		While *n < tlen
			...
			If __TS.is_suffix_char(this_char, TRUE) = TRUE Then	 
				_tokenize_suffix(t_text, t, this_char, TRUE)
				Return TRUE 
	...
	End Function
	Function _is_number_string(ByRef tkn As TToken) As boolean
		'
		Enum NumberModes
			nmNone
			nmBase10
			...
			*n += 1
			If *n < tlen Then
				...
				If this_char = 98 Then 	' b
					t = ttBinNumber
					t_text &= this_char
					If _is_binary_string(t) = TRUE Then
						 tkn = t
						Return TRUE
					EndIf
					*n = orig_n
	...
	Sub _tokenize_line_cont()
		...
		*n += 1
		...
		t.line_number = _char_line		' we are storing ignore text that comes after _
		t.col = _char_col
		t.token_type = ttNone
		While *n < tlen
			this_char = z[*n][0]
	...
	'	'
		...
		If __TS.find_word(t_text) <> 0 Then
		...
		If get_low_case(t_text) = "enum" Then
			...
			_add_enum(tokens.get_last_item())		' the array will be filled with nodes that are the enum
		EndIf
	
		
	End Sub
	Function _add_enum(ByVal node As TListNode Ptr) As UShort
		'
		'Dim As TListNode Ptr t
		_enum_count += 1
		ReDim Preserve _enum_nodes(1 To _enum_count) 
		_enum_nodes(_enum_count) = node
		Return _enum_count
	End Function
			...
			If mode = mdNone Then
				...
				ElseIf this_char = 38 Then		' & - this could be a hex binary oct number
					mode = mdNumber
						t.col = _char_col 
						t.line_number = _char_line
						If _is_number_string(t) = TRUE Then
							tokens.add_token(@t)
						Else 
				...
				Else
					? "Error: Unhandled char '" & this_char & "'" & _char_line & " at column " & _char_col 
					? __ERROR_INFO				 
					Sleep
					End
					...
					If (*n + 1) < tlen Then
						*n += 1
						...
						Else					
							...
							*n -= 1
							t.index.last_char = *n - 1
							tokens.add_token(@t)  	' add the quoted literal 
